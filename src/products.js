export const products = [
    {
        id: 0,
        category: "Engine & Mechanical",
        subcategory: "Suspension",
        manufacturer: "Honda",
        vehicle: "Honda Civic",
        vehiclemodel: ["2006", "2007", "2008", "2009", "2010", "2011", "2012"],
        title: "Inner Axle HDK Japan For Honda Civic 2007-2012 LH",
        desc: "Japan made outer axle for honda civic reborn",
        rating: 4,
        price: 14500,
        featured: true,
        img: ["https://cache1.pakwheels.com/ad_pictures/2511/inner-axle-hdk-japan-for-honda-civic-2007-2012-lh-25113275.jpg"]

    },
    {
        id: 1,
        category: "Brakes",
        subcategory: "Brake Pads",
        manufacturer: "Akebono",
        vehicle: "Suzuki Wagon R Japanese",
        vehiclemodel: [],
        title: "Suzuki Wagon R Japanese Akebono Front Brake Pad",
        desc: "Ultra-Premium Ceramic Front Disc Brake Pads. These pads combine OE quality and fit with application-specific friction formulations to deliver a superior level of braking performance. Innovative ceramic formulations virtually eliminate brake squealing, groaning, and vibration while extending rotor life for smooth braking performance you can trust.",
        rating: 4,
        price: 5700,
        featured: true,
        img: ["https://cache2.pakwheels.com/ad_pictures/5460/suzuki-wagon-r-japanese-akebono-front-brake-pad-54607451.jpg"]
    },
    {
        id: 2,
        category: "Car Care",
        subcategory: "Coolants",
        manufacturer: "Cyclo",
        vehicle: "",
        vehiclemodel: [],
        title: "Cyclo Coolant Booster - 1 Litre",
        desc: "Extends coolant life Restores additives contained in a full gallon of quality anti-freeze Lubes seals and moving parts Preserves aluminum alloy and other materials in cooling system",
        rating: 4,
        price: 1800,
        img: ["https://cache2.pakwheels.com/ad_pictures/3642/cyclo-coolant-booster-1-litre-36429874.jpg"]
    },
    {
        id: 3,
        category: "Interior",
        subcategory: "Back & Neck Care",
        manufacturer: "WMS",
        vehicle: "",
        vehiclemodel: [],
        title: "Car Back Rest | Car Back Care Cushion | Lumbar Support | Black",
        desc: "75 degree posterior tilt design, maintain the natural curvature of the lumbar spine, protect the lumbar vertebrae to avoid nerve compression, and naturally fit to release body pressure. 3D lumbar support, enjoy decompression: the natural curve design of the back of the human body, soothing the waist and soreness. The two sides are surrounded by a design, which fits the muscles on both sides of the back, and the space for releasing the lumbar vertebrae is relieved.",
        rating: 4,
        price: 1500,
        featured: true,
        img: ["https://cache4.pakwheels.com/ad_pictures/7636/car-back-rest-car-back-care-cushion-lumbar-support-black-76363173.webp",
            "https://cache1.pakwheels.com/ad_pictures/7636/car-back-rest-car-back-care-cushion-lumbar-support-black-76363174.webp",
            "https://cache2.pakwheels.com/ad_pictures/7636/car-back-rest-car-back-care-cushion-lumbar-support-black-76363175.webp"
        ]
    },
    {
        id: 4,
        category: "Exterior",
        subcategory: "Viper Blades",
        manufacturer: "Honda",
        vehicle: "BR-V",
        vehiclemodel: ["2017", "2018", "2019", "2020", "2021", "2022"],
        title: "Honda BR-V 2017-2022 Silicone Wiper Blades",
        desc: "",
        rating: 4,
        price: 1000,
        featured: false,
        img: ["https://cache4.pakwheels.com/ad_pictures/6390/honda-br-v-2017-2022-silicone-wiper-blades-soft-rubber-vipers-graphite-coated-rubber-63902171.jpg"]
    },
    {
        id: 5,
        category: "Lights & Electrical",
        subcategory: "LED Lights",
        manufacturer: "Universal",
        vehicle: "",
        vehiclemodel: [],
        title: "Universal Headlight DRL With Indicator",
        desc: "",
        rating: 4,
        price: 1700,
        featured: false,
        img: ["https://cache3.pakwheels.com/ad_pictures/5483/universal-headlight-drl-with-indicator-54833199.jpg"]
    },
    {
        id: 6,
        category: "Oil & Lubricants",
        subcategory: "Engine Oil",
        manufacturer: "ZIC",
        vehicle: "",
        vehiclemodel: [],
        title: "Zic Engine Oil 0W-30 - 4 Litre",
        desc: "ZIC X7 0W-30 (VHVI TECH) is a synthetic formula engine oil coupled with the latest APISN grade, and is engineered with SKâ€™s proprietary technology, VHVI TECH. ZIC X7 0W-30 (VHVI TECH) delivers exceptional fuel economy benefits and engineprotection compared to conventional oil viscosity grades such as SAE 20W-50.ZIC X7 0W-30 (VHVI TECH) has a longer draining interval compared to conventional oils, which enables you to extend your vehicle service interval. It also provides outstanding engine protection, and is recommended to be used in Japanese, Korean, American cars where API SN/SM/SL/SG is recommended.",
        rating: 4,
        price: 8000,
        featured: false,
        img: ["https://cache2.pakwheels.com/ad_pictures/4353/zic-engine-oil-0w-30-4-litre-43533830.jpg"]
    },
    {
        id: 7,
        category: "Tools & Gadgets",
        subcategory: "Key Covers",
        manufacturer: "",
        vehicle: "Honda Civic",
        vehiclemodel: ["2016", "2017", "2018", "2019", "2020", "2021"],
        title: "Honda Civic 2016-2021 Carbon Key Case",
        desc: "",
        rating: 4,
        price: 1800,
        featured: false,
        img: ["https://cache4.pakwheels.com/ad_pictures/5598/honda-civic-2016-2021-carbon-key-case-55985460.jpg"]
    },
    {
        id: 8,
        category: "Tyres & Wheels",
        subcategory: "Car Tyres",
        manufacturer: "Nexen",
        vehicle: "",
        vehiclemodel: [],
        title: "Nexen Tire Roadian HP 285/60R18",
        desc: "This model offers a longer lasting tread life. The tire's tread wear rate is slowed down as the footprint maintains constant contact with the road and evenly distributes the driving pressure along the tread area. The constant contact equally spreads the force of acceleration, cornering and braking out across the tire’s surface and prevents irregular wear on the tire, extending its life.",
        rating: 4,
        price: 55500,
        featured: false,
        img: ["https://cache2.pakwheels.com/ad_pictures/5598/nexen-tire-roadian-hp-285-60r18-55986890.jpg",
            "https://cache2.pakwheels.com/ad_pictures/5598/nexen-tire-roadian-hp-285-60r18-55986890.jpg",
            "https://cache2.pakwheels.com/ad_pictures/5598/nexen-tire-roadian-hp-285-60r18-55986890.jpg",
            "https://cache1.pakwheels.com/ad_pictures/5598/nexen-tire-roadian-hp-285-60r18-55986893.jpg"]
    },
    {
        id: 9,
        category: "Audio & Video",
        subcategory: "Audio Accessories",
        manufacturer: "",
        vehicle: ["Honda City", "Honda Grace", "Honda Vezel"],
        vehiclemodel: [],
        title: "Honda City 2022 Multimedia Steering Wheel",
        desc: "",
        rating: 4,
        featured: false,
        img: ["https://cache2.pakwheels.com/ad_pictures/7488/honda-city-2022-multimedia-steering-wheel-buttons-keys-grace-audio-buttons-74884589.webp",
            "https://cache4.pakwheels.com/ad_pictures/7488/honda-city-2022-multimedia-steering-wheel-buttons-keys-grace-audio-buttons-74884590.webp",
            "https://cache2.pakwheels.com/ad_pictures/7488/honda-city-2022-multimedia-steering-wheel-buttons-keys-grace-audio-buttons-74884591.webp"]
    }
];

export const deals = [
    {
        id: 2,
        category: "Car Care",
        subcategory: "Coolants",
        manufacturer: "Cyclo",
        vehicle: "",
        vehiclemodel: [],
        title: "Cyclo Coolant Booster - 1 Litre",
        desc: "",
        rating: 4,
        price: 1800,
        img: ["https://cache2.pakwheels.com/ad_pictures/3642/cyclo-coolant-booster-1-litre-36429874.jpg"]
    },
    {
        id: 3,
        category: "Interior",
        subcategory: "Back & Neck Care",
        manufacturer: "WMS",
        vehicle: "",
        vehiclemodel: [],
        title: "Car Back Rest | Car Back Care Cushion | Lumbar Support | Black",
        desc: "",
        rating: 4,
        price: 1500,
        featured: true,
        img: ["https://cache4.pakwheels.com/ad_pictures/7636/car-back-rest-car-back-care-cushion-lumbar-support-black-76363173.webp",
            "https://cache1.pakwheels.com/ad_pictures/7636/car-back-rest-car-back-care-cushion-lumbar-support-black-76363174.webp",
            "https://cache2.pakwheels.com/ad_pictures/7636/car-back-rest-car-back-care-cushion-lumbar-support-black-76363175.webp"
        ]
    }
]

export const recommendedProducts = [
    {
        id: 0,
        category: "Engine & Mechanical",
        subcategory: "Suspension",
        manufacturer: "Honda",
        vehicle: "Honda Civic",
        vehiclemodel: ["2006", "2007", "2008", "2009", "2010", "2011", "2012"],
        title: "Inner Axle HDK Japan For Honda Civic 2007-2012 LH",
        desc: "",
        rating: 4,
        price: 14500,
        featured: true,
        img: ["https://cache1.pakwheels.com/ad_pictures/2511/inner-axle-hdk-japan-for-honda-civic-2007-2012-lh-25113275.jpg"]

    },
    {
        id: 1,
        category: "Brakes",
        subcategory: "Brake Pads",
        manufacturer: "Akebono",
        vehicle: "Suzuki Wagon R Japanese",
        vehiclemodel: [],
        title: "Suzuki Wagon R Japanese Akebono Front Brake Pad",
        desc: "",
        rating: 4,
        price: 5700,
        featured: true,
        img: ["https://cache2.pakwheels.com/ad_pictures/5460/suzuki-wagon-r-japanese-akebono-front-brake-pad-54607451.jpg"]
    }
];

export const featuredProducts = [
    {
        id: 0,
        category: "Engine & Mechanical",
        subcategory: "Suspension",
        manufacturer: "Honda",
        vehicle: "Honda Civic",
        vehiclemodel: ["2006", "2007", "2008", "2009", "2010", "2011", "2012"],
        title: "Inner Axle HDK Japan For Honda Civic 2007-2012 LH",
        desc: "",
        rating: 4,
        price: 14500,
        featured: true,
        img: ["https://cache1.pakwheels.com/ad_pictures/2511/inner-axle-hdk-japan-for-honda-civic-2007-2012-lh-25113275.jpg"]

    },
    {
        id: 1,
        category: "Brakes",
        subcategory: "Brake Pads",
        manufacturer: "Akebono",
        vehicle: "Suzuki Wagon R Japanese",
        vehiclemodel: [],
        title: "Suzuki Wagon R Japanese Akebono Front Brake Pad",
        desc: "",
        rating: 4,
        price: 5700,
        featured: true,
        img: ["https://cache2.pakwheels.com/ad_pictures/5460/suzuki-wagon-r-japanese-akebono-front-brake-pad-54607451.jpg"]
    }
];

export const cartItems = [
    {
        quantity: 1,
        product: {
            id: 4,
            category: "Exterior",
            subcategory: "Viper Blades",
            manufacturer: "Honda",
            vehicle: "BR-V",
            vehiclemodel: ["2017", "2018", "2019", "2020", "2021", "2022"],
            title: "Honda BR-V 2017-2022 Silicone Wiper Blades",
            desc: "",
            rating: 4,
            price: 1000,
            featured: false,
            img: ["https://cache4.pakwheels.com/ad_pictures/6390/honda-br-v-2017-2022-silicone-wiper-blades-soft-rubber-vipers-graphite-coated-rubber-63902171.jpg"]
        }
    },
    {
        quantity: 2,
        product: {
            id: 6,
            category: "Oil & Lubricants",
            subcategory: "Engine Oil",
            manufacturer: "ZIC",
            vehicle: "",
            vehiclemodel: [],
            title: "Zic Engine Oil 0W-30 - 4 Litre",
            desc: "",
            rating: 4,
            price: 8000,
            featured: false,
            img: ["https://cache2.pakwheels.com/ad_pictures/4353/zic-engine-oil-0w-30-4-litre-43533830.jpg"]
        }
    },
    {
        quantity: 4,
        product: {
            id: 8,
            category: "Tyres & Wheels",
            subcategory: "Car Tyres",
            manufacturer: "Nexen",
            vehicle: "",
            vehiclemodel: [],
            title: "Nexen Tire Roadian HP 285/60R18",
            desc: "This model offers a longer lasting tread life. The tire's tread wear rate is slowed down as the footprint maintains constant contact with the road and evenly distributes the driving pressure along the tread area. The constant contact equally spreads the force of acceleration, cornering and braking out across the tire’s surface and prevents irregular wear on the tire, extending its life.",
            rating: 4,
            price: 55500,
            featured: false,
            img: ["https://cache2.pakwheels.com/ad_pictures/5598/nexen-tire-roadian-hp-285-60r18-55986890.jpg",
                "https://cache2.pakwheels.com/ad_pictures/5598/nexen-tire-roadian-hp-285-60r18-55986890.jpg",
                "https://cache2.pakwheels.com/ad_pictures/5598/nexen-tire-roadian-hp-285-60r18-55986890.jpg",
                "https://cache1.pakwheels.com/ad_pictures/5598/nexen-tire-roadian-hp-285-60r18-55986893.jpg"]
        }
    },
];